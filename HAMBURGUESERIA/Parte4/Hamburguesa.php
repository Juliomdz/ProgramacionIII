...
class Hamburguesa {
    ...
    * y resultado de operacion: TRUE si ok, FALSE si no hay stock o error */
    static public function UpdateArrayByVenta($arr = array(), $Hamburguesa, $cupon){
        $return = false;
        if (is_array($arr) && count($arr) > 0 && $Hamburguesa->VerifyNombreTipo($arr))
        {
            foreach ($arr as $objValue) 
            {
                if ($Hamburguesa->__Equals($objValue)) 
                {
                    ...
                    if ($cupon== true) {
                        $objValue->precio = $objValue->precio * 0.9;
                        echo "Actualizado el precio por el cupon aplicado:$$objValue->precio", PHP_EOL;
                    }
